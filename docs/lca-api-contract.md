...
- no `include_user_unassigned_only` or `include_user_review_free_only` guard; `team_id` and `review_id` are workflow metadata, not ownership gates;
- `scope_manifest` using `lca.data_scope.manifest.v2`, predicate `public_state_100_or_authenticated_owner_state_0.v2`, and its canonical `scope_manifest_sha256`;
...
The worker independently enforces the frozen predicate after queue decoding and again in the snapshot builder. The data scope applies only to processes and flows: they are eligible only when `state_code=100`, or when they are actor-owned `state_code=0` rows. Team and review metadata do not remove an owner-readable draft. Public states `101..199`, foreign drafts, and owner nonzero rows are rejected. LCIA methods are intentionally outside this actor-specific DB predicate. During rollout, the worker also reads the frozen v1 manifest with its original null-team/null-review guards; v1 and v2 remain distinct through their manifest hashes.

#### 3.7.0 Public numerical Process eligibility
Public numerical eligibility is exactly `state_code = 100`. The reserved publication segment `100..=199` is a reserved metadata range, not a computation capability and not a generic display/publication grant, so an unknown or newly reserved public state fails closed rather than being treated as "published therefore computable".

A published Result Process (`state_code = 120`) is **not** exposed through generic product reads, references or exports. It is neither a numerical root, provider nor matrix axis, and it is excluded from product export packages (including `open_data` packages, since `120` lies inside their existing `100..=199` predicate and is removed by an exact exclusion). Its only readback is the constrained, authorization-bound receipt path owned by the Database Result publication command; broadening that into a generic public Result read or export is a separate product decision with its own owning task. Support data (Flow, FlowProperty, UnitGroup, Source, Contact) keeps its existing `100..=199` semantics unchanged.

Every numerical entrypoint enforces this independently rather than relying on one shared range filter:

- the offline `snapshot-builder` default (`--process-states` default value `100`) resolves to exactly `100`; an explicit `all`/empty list is treated as that same fail-closed scope, an explicit `--all-states true` is rejected, and an explicit list naming any other state (including `0`, `20` or `120`) fails closed;
- a queued `lca.build_snapshot.request.v1` payload must set `all_states=false` and either omit `process_states` or pass exactly `100`; `lca.build_snapshot.request.v2` keeps its frozen `process_states="100"` requirement;
- the data-product package `inputManifest.processes` accepts `stateCode/state_code` exactly `100` for live builds, and a certificate-owned axis that still carries state metadata naming `120` fails closed;
- certified Scope Closure and data-product package snapshot builds pin exactly `100` rather than inheriting an offline default.

Owner drafts remain authorized only by the versioned `public_plus_owner_draft` scope and only for the actor that owns the row; a state value is never accepted as evidence of scope authorization. That authorization applies identically to **explicit request roots**: an explicit `--root-process` naming a state-0 Process is admitted only when the resolved row is owned by the invocation's scoped actor and the same team/review guards hold, while a foreign draft, an ownerless draft, a published Result, and an unknown/absent state all fail closed with `request_root_not_numerically_eligible`. The two actor contexts are distinct and must not be conflated: the versioned `public_plus_owner_draft` path proves an **authenticated** actor through the queue contract, whereas the offline `snapshot-builder --include-user-id` path only proves an **operator-supplied** identity with no authentication; it is an operator tool, not a user-facing authorization boundary. The dedicated Review Admin quality diagnostic is the single intentional numeric exception (in-review `20` plus public `100`) and never dispatches through a generic job path. Administrative, export/import and lineage reads keep their own visibility rules and are unaffected.

Every newly built numerical snapshot records a global versioned numerical-policy marker, `public-numerical-state-100-excluding-result-120:v1`. New compute execution and snapshot reuse require that exact marker: an artifact that declares it missing, stale or unknown fails closed, and a missing marker is never treated as implicitly current. This covers ordinary global/subset snapshots as well as closure-bound ones, because an earlier ordinary snapshot may have been selected under the retired `100..199` membership and could contain a published Result or other reserved states. The marker also participates in the source fingerprint that drives snapshot/cache identity, so an artifact built under a different policy can never match a new build or reuse lookup. Historical artifacts stay byte-identical and readable through administrative readers, which never consult the marker; readable history is simply not new-compute eligible.

Closure-bound snapshots additionally record `lcia.numerical-snapshot-build-contract.v1`, the binding hash over the exact scope-closure evidence, snapshot identity and artifact format, matching the Database-side derivation. That binding hash is supplementary evidence for closure-bound artifacts only: it can never replace the global policy marker, and it is bound to a persisted snapshot id that is never invented for reuse. Certified package reuse additionally requires a current Database eligibility predicate: a manifest selected under a retired reserved-range predicate, or one declaring none, must be rebuilt instead of silently re-executed. The consumed Database literals are `candidate-public-state-code-100:v2` (no-current-release closure candidate universe) and `published-state-code-100:latest-per-id:v2` (eligible-input manifest); `current-public-release-manifest:v2` remains the separate formal-release membership contract.

The dedicated Review Admin quality diagnostic admits only in-review `20` and public `100`; the package-build manifest path also fails closed for a certified axis whose state metadata names the published Result state.

#### 3.7.2 Stored READY snapshot policy marker (producer contract)

The Worker authors `private.lca_network_snapshots.process_filter` for READY snapshots and now always persists `numerical_policy_version` in that object, with the exact value `public-numerical-state-100-excluding-result-120:v1`. The value is derived from the Worker constant on every branch (versioned `public_plus_owner_draft` scope, ordinary public scope, `all_states`, and owner-inclusive scope); it is never seeded from caller input, so a proposed build filter cannot authorize a READY artifact, and a caller-supplied field is not part of any job payload schema.

This uses the existing transport only: `api.svc_lca_snapshot_candidates` already returns `processFilter` verbatim, so there is no new Database column or RPC. A consumer that accepts a stored READY snapshot for a NEW calculation — automatic lookup or explicit snapshot before a cached result is returned — must require the current marker, must keep requiring the artifact to be READY, and must not infer freshness from `100`-shaped states. Historical read-only artifact routes are unaffected.

Worker-side tests assert the marker on every scope branch and assert that a missing or retired marker is not current. The consumer-side validation, including cached-result reuse, is owned by the Edge task and is not proven here.
...
新建 certificate-grade Scope Closure 的 `linkPolicy.technosphereBoundaryPolicy` 固定冻结为 `cutoff`。数据库对兼容输入中的省略值、`closed`、`open` 和 `cutoff` 统一规范化后再计算 scope/policy/request hash；Worker 只接受冻结后的 `cutoff`，不会在运行时静默覆盖不同策略。非 canonical 输入在 scan/publication 前以 `scope_closure_boundary_policy_must_be_cutoff` 失败。`cutoff` 下 unmatched provider、A-write coverage、unresolved balance 和 medium singular risk 继续进入 metrics、findings 与逐边 evidence；medium 以 `matrix_readiness_singular_risk_medium` warning 保留但不成为 certificate blocker，high singular risk、factorization/sample solve 失败和非有限计算仍 fail closed。generic snapshot/readiness CLI 的三策略兼容面与 medium 默认 blocker 语义不变。
...
- `tiangong.calculation-bundle.process-axis.v2` 固化 Process UUID/version 和唯一 quantitative reference 的 exchange internal ID、Flow UUID/version、reference unit，以及 `pivot.rawDirection`、`pivot.rawMeanAmount`、`pivot.signedRawCoefficient`、`pivot.normalizationScale`、`pivot.normalizedCoefficient`。其中 `signedRawCoefficient = directionSign × rawMeanAmount`、`normalizationScale = 1 / abs(signedRawCoefficient)`，顶层 `meanAmount = rawMeanAmount × normalizationScale`；Bundle 构造时逐项校验并 fail closed。该 pivot 是下游重建 Result Process 与 LifecycleModel 的计算时证据，不能统一改写为 Output 或要求消费者重新查询数据库。Fresh release evidence 必须完整生成 v2；兼容读取的历史 snapshot 若所有 Process 都没有这些字段，仍生成 process-axis v1，交由下游结合同一 Bundle 的 exact source closure 处理；部分 Process 缺失或字段只填一部分属于损坏证据并 fail closed。inventory axis逐 exchange 保存 raw/signed/normalized coefficient、allocation target 与 selected fraction。Snapshot flow axis 使用 `(Flow UUID, resolved version)`；同一 UUID 的多个实际引用 revision 可共存并获得独立连续 `flow_idx`，未被最终 process closure exchange 引用的 revision 不进入矩阵。LCIA factor 只有在其 Flow 与 inventory-derived biosphere axis 相交且实际进入 C 时才成为数值 source-closure 依赖；off-axis factor 不创建额外 Flow axis 或 support root。

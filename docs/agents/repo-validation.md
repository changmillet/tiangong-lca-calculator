...
| finite frozen-input provider impact | `cargo test -p solver-worker --bin snapshot_builder provider_impact::tests`; `cargo test -p solver-worker --test provider_impact_cli`; full snapshot-builder, readiness and signed-flow tests; `make check`; hard Clippy/format and Docpact gates | exercise the public command with a synthetic singular matrix and invalid DB/storage configuration; require zero factorization/unit solves, deterministic full-source report bytes, raw input/binary/baseline/candidate hash binding and no output overwrite. Cover disconnected and transitive consumers, exact Process/Flow revisions, omitted-version freezing, lineage conflict, missing support and forbidden overlays. | This proves the supplied finite universe only. Source closure, online census, authorized input acquisition, cross-repository integration, TIDAS schema/scientific checks, numerical qualification and native production retest remain separate. See `docs/provider-impact-diagnostic-contract.md`. |

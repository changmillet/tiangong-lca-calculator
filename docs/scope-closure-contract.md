...
The current internal request identity is `scope-closure-validator-scanner.v1+cutoff-readiness-r4`. Worker also accepts the historical exact values `scope-closure-validator-scanner.v1+cutoff-readiness-r3`, `scope-closure-validator-scanner.v1+cutoff-readiness-r2`, `scope-closure-validator-scanner.v1+cutoff-readiness-r1`, and `scope-closure-validator-scanner.v1` so already-enqueued and in-flight requests remain executable during rollout. This is a closed compatibility set: any other fingerprint fails before scan claim. The revision changes cache identity only; it does not version or alter the public V1 job, result, certificate, or artifact contracts.
...
The Worker coalesces deterministic issue keys while retaining exact deduplicated occurrence counts and orders the final set by stable `issueKey`. The canonical set is unified across TIDAS, exact-reference and graph findings, frozen-release/source-drift findings, snapshot source-preflight findings, provider findings, matrix/factorization findings, and LCIA-readiness blockers. Snapshot source blockers use `code + requested target identity` as issue identity; source identity and JSON path remain occurrences, so repeated paths to the same missing target do not create independent findings. If the target is malformed, the raw target fingerprint and extraction code replace the parsed identity. Each `lcia.scope-closure-issue.v3` main record retains the stable semantic fields, severity, blocker flag, exact `occurrenceCount`, exact `affectedRootCount`, and explicitly bounded occurrence/root samples. A grouped multi-source issue has no misleading single top-level source/path. Only the bounded RPC/general XLSX projection is retained in `scan.issues`; the manifest artifacts are the completeness authority. Snapshot source-preflight blockers additionally receive complete dedicated XLSX worksheets from their verified file-backed stream.

Graph analysis groups the sorted merge stream by issue identity, walks its sources in stable order, and keeps only one source's reverse-reachability state plus one root bitset for the current issue. It unions affected roots exactly and writes a compact issue-level root-impact record using stable ordinals with explicit `none`, `allRoots`, `includedOrdinals`, or `excludedOrdinals` mode. The frozen exact reference graph stores compact reverse predecessors once, allowing a reader to reconstruct sampled witnesses deterministically. Production emits zero expanded affected-root rows and no repeated full witness paths. The V3 result RPC receives exact counts, at most 5,000 issue summaries, and at most 100 occurrences/affected roots per projected issue; truncation flags make sampling explicit.
...
4. evaluate the discovered matrix, provider-link, factorization, and LCIA readiness evidence; certificate-grade medium singular risk is retained as `matrix_readiness_singular_risk_medium` with `severity=warning` and `blocking=false`, while high singular risk and actual compute failures remain blockers;

5. merge typed snapshot source blockers into the canonical issue stream; only when every scan is complete and no blocker remains, run the frozen snapshot builder in persisted build mode;
6. determine the terminal status, build the report once, and publish the staged artifact write-set.

Administrative and final closure bundles, document/edge/reference evidence, canonical issue partitions, compact graph/impact evidence, XLSX worksheets, and object-store uploads are file-backed. Canonical V1 arrays are emitted incrementally from indexed or sorted spools; the Worker does not reconstruct complete document, reference-edge, TIDAS issue-event, snapshot blocker, resolution-map, root-impact, or witness collections as `Vec<Value>`. A blocked `snapshot_builder_terminal.v1` keeps at most 16 diagnostic samples and adds only a `recordCount`/`byteSize`/`sha256`/`collectionComplete` descriptor for a parent-owned canonical NDJSON sidecar. Worker validates all four bindings plus the terminal aggregate hash before streaming the sidecar into issue coalescing. A successful discovery likewise never embeds the discovered axis or readiness report in captured stdout: the parent provides one temporary JSON path, the child writes only `processAxis` plus the compact readiness `schema_version`, `status`, `next_action`, `findings`, and `blockers` fields, and the terminal carries only its `byteSize` and `sha256`. The parent enforces the file-size ceiling, verifies both bindings, and parses the file before continuing. The full readiness report remains owned by its existing report path and is not duplicated across this child-process boundary. Missing, incomplete, malformed, truncated, count-mismatched, or hash-mismatched result files are protocol failures rather than domain blockers. Temporary directories own every intermediate spool/run/artifact and remove them on success, failure, cancellation, or lease loss after the bounded blocking task exits. Cancellation is checked during merge, coalescing, reachability, partition writing, frozen-graph writing, TIDAS compression, and between report stages. Lease heartbeats remain active during artifact preparation and each upload.
...
- `closure-report-v1.xlsx`: a valid workbook tagged with the current `closureCheckId`, containing exact summary counts, a complete-artifact index, at most 5,000 general issue rows, 10,000 general occurrence rows, and 10,000 general affected-root rows. When snapshot source preflight blocks, dedicated `Snapshot Blockers N` worksheets contain every verified sidecar record and split before the Excel row or worksheet-byte limit; they are never silently truncated.
...
Every prepared scope-closure object is checked against a 256 MiB hard ceiling before a write-set header is created, so an oversized physical object cannot be registered, sealed, or uploaded. A logical administrative record may exceed that ceiling only because its physical representation is bounded to one small index and 8 MiB chunks. Before opening the XLSX ZIP, Worker splits snapshot-blocker data before a worksheet reaches 1,048,576 rows or 64 MiB estimated uncompressed XML and rejects any other worksheet above those limits. It does not reject the sum of streamed worksheet XML sizes; the finished compressed archive retains its 64 MiB hard limit. Snapshot blocker rows contain structured columns once and do not duplicate the complete reason JSON.
...
For a complete blocker-free run, the existing frozen `snapshot_builder` persists the real numerical `snapshot-hdf5:v1` artifact and snapshot-index sidecar through `lca_network_snapshots` and `lca_snapshot_artifacts`. Passed evidence comes back from those persisted records and binds `snapshotId`, the HDF5 artifact SHA-256 as `snapshotHash`, `snapshotArtifactId`, `snapshotIndexSha256`, and `snapshotBuildContractHash`. The embedded HDF5 binding uses `lcia.scope-closure-snapshot-binding.v1` and binds `effectiveScopeHash`, `dataSnapshotToken`, and `closureBundleHash`. The snapshot-index sidecar is the authoritative exact ordered Process axis; its count must also match the numerical payload. Calculation Bundle metadata and immutable source documents are separate integrity-bound zstd artifacts: HDF5 binds release-evidence v2, and release evidence binds the content-addressed source closure by URL/SHA-256/size/format/dataset count. No layer persists a full compiler graph or duplicates source documents. Generic live-snapshot reuse cannot substitute an artifact that lacks these bindings. A persisted numerical snapshot additionally records the global numerical-policy marker `numerical_policy_version = public-numerical-state-100-excluding-result-120:v1` in its artifact config and in the server-authored READY `process_filter`. That marker is the policy identity for public numerical eligibility (exact `100`); the binding hash above stays specific to the frozen scope-closure evidence. New compute execution, generic live-snapshot reuse, and certified package reuse all require the current marker in addition to their own binding checks, so a snapshot produced under the retired `100..199` membership fails closed even though it still decodes. Historical artifacts remain byte-identical and readable through administrative readers, which never consult the marker.
